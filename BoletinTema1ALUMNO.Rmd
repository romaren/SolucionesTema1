...
# c)
r_m<- rownames(mammals)
r_m
colnames(mammals)

r_a<- rownames(Animals2)
r_a
colnames(Animals2)
```
...
```{r}
#Animales en mammals que no están en Animals2
diferentes_mammals<-setdiff(r_m,r_a)
diferentes_mammals
#Son 4

#Animales en Animals 2 que no están en mammals
diferentes_anim2<-setdiff(r_a,r_m)
diferentes_anim2
#Son 7 animales
```
...
```{r}
#Los nombres están escritos diferentes, pero se refieren al mismo animal ya que tienen las mismas características
```
...
#Presentan prácticamente el mismo comportamiento, y como las muestras están dentro de las líneas discontinuas, podemos decir que si siguen una distribución Gaussiana
```
...
#b
#Muestro el nombre de las librerías 
libReturn[[2]][1:5]
#Muestro los primeros 5 elementos 
head(libReturn$results,n=5)

#c
length(libReturn$results)
#Tengo 654 librerías instaladas
...
#a
str(cabbages)
#60 observaciones de 4 variables
nrow(cabbages)
#60 filas
ncol(cabbages)
#4 columnas
...
#c
str(Chile)
#2700 observaciones de 8 variables
nrow(Chile)
#2700 filas
ncol(Chile)
#8 columnas
...
summary(cabbages)
summary(Chile)
#Vemos que cabbages no tiene ningún valor perdido, pero Chile sí
```
...
```{r}
#Representación de vitamina C en función del peso de la calabaza
plot(cabbages$VitC ~ cabbages$HeadWt, xlab='Peso Calabaza', ylab='Vitamina C')
#No existe una relación lineal entre ellas
```
...
```{r}
##Representación de vitamina C en función del peso de la calabaza por variedad de calabaza
plot(cabbages$VitC ~ cabbages$HeadWt, pch=c(1,18)[cabbages$Cult])

#Modelo ambos
Mod_ambos<-lm(cabbages$VitC ~ cabbages$HeadWt, data=cabbages)
#obtenemos el modelo c39
Modeloc39<-lm(cabbages$VitC ~ cabbages$HeadWt, data=cabbages, subset=which(Cult=="c39"))

#obtenemos el modelo c52
Modeloc52<-lm(cabbages$VitC ~ cabbages$HeadWt, data=cabbages, subset=which(Cult=="c52"))

#Dibujamos las rectas de la relación
abline(Modeloc39, lty=3)
abline(Modeloc52)
abline(Mod_ambos, col='green')
...
```{r}
#Mostramos el resumen de los dos modelos
summary(Modeloc39)
summary(Modeloc52)
#Obtenemos la intercepción y la pendiente
lm(Modeloc39)
lm(Modeloc52)
#En efecto, vemos que las pendientes son negativas
```
